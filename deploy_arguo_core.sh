#!/bin/bash
#==============================================================================
# OpenList + qBittorrent + Aria2 One-Click Deploy — Core (arguo's config)
# Usage: bash deploy_arguo_core.sh  (root user)
#
# TWO-STEP DEPLOYMENT:
#   1. Run deploy_arguo_ssl.sh FIRST to obtain SSL certificates
#   2. Take a VM snapshot
#   3. Run this script (deploy_arguo_core.sh) for everything else
#
...
if [ "$HAS_DOMAIN" = "yes" ]; then
    info "SSL certificates should already be set up by deploy_arguo_ssl.sh"
    if [ -f "/etc/letsencrypt/live/$DOMAIN/fullchain.pem" ]; then
        ...
        warn "SSL cert not found at /etc/letsencrypt/live/$DOMAIN/"
        warn "Run deploy_arguo_ssl.sh first to obtain certificates"
    fi
else
    info "No domain configured; skipping HTTPS setup"
